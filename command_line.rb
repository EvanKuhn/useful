...
class CommandLine
  attr_reader :appname, :description, :examples
  attr_reader :help
...
cli.option(:warning, '-w', '--warning PCT', 'This is the warning threshold, 0-100', 666) { |v| Float(v) }
cli.option(:flag, '-f', '--flag', 'A flag')
cli.option(:bar, '-b', nil, 'Another flag')

#puts cli.usage
cli.parse

puts "warning = #{cli.warning}"
puts "flag = #{cli.flag}"
puts "bar = #{cli.bar}"
